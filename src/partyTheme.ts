...
function buildPartyThemePrompt(form: PartyForm): string {
  return `Plan a party theme for:
...
export async function fetchPartyThemeFromGemini(form: PartyForm, apiKey: string): Promise<PartyTheme> {
  const response = await fetch(GEMINI_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      contents: [{
        role: 'user',
        parts: [{ text: buildPartyThemePrompt(form) }],
      }],
    }),
  });
